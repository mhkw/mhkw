...
export default class Address extends React.Component {
    ...
    componentDidMount() {
        ...
        this.LocalSearch = new BMap.LocalSearch(city, { //智能搜索
            onSearchComplete: this.searchInCity
        });
        this.LocalSearch.setPageCapacity(10);
    ...
    onClickSelectAddr = (value) => {
        ...
        let lon = value.point.lng;
        let lat = value.point.lat;
    ...
    //定位当前位置,拿到城市，然后设置进state
    getLocationCity = () => {
        let then = this;
        this.geolocation.getCurrentPosition(function (r) {
            if (this.getStatus() == BMAP_STATUS_SUCCESS) {
                then.geoc.getLocation(r.point, (rs) => {
                    ...
                    then.LocalSearch.setLocation(city)
                });
    ...
    getNameFromCoords = (point) => {
        ...
        this.geoc.getLocation(point, (rs) => {
            ...
            let address = district + street + streetNumber;
            let lon = point.lng;
            let lat = point.lat;
            let currentLocation = street + streetNumber;
            //保存到历史记录里去
            this.addHistoryAddress({
                city,
                address,
    ...
    searchInCity = (keyword, pageIndex = 0, pageCapacity = 10) => {
        ...
        // let results = this.Autocomplete.getResults();
        let results = this.LocalSearch.getResults().tr; //返回的结果
        let totalPage = this.LocalSearch.getPageCapacity(); //当前页数量

        this.setState({
            searchInCity_results: results, //搜索
            searchInCity_pageIndex: 0, //搜索的结果的当前页数索引,只搜索第一页10个
            searchInCity_totalPage: totalPage, //搜索的结果的总页数,获得第一页的数量也行
        })
