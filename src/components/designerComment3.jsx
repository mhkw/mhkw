...
import { Toast, NavBar, Icon, Flex, TextareaItem, Button } from 'antd-mobile';
...
export default class DesignerComment extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            commentKeywords: [],
            // commentList: [], //评论列表
            commentList: sessionStorage.getItem("commentList") ? JSON.parse(sessionStorage.getItem("commentList")) : [], //评论列表
    ...
    ajaxGetCommentList = ( per_page = 5, page = 1 ) => {
        ...
        if (this.props.location.query && this.props.location.query.id) {
            article_id = this.props.location.query.id;
        }
    ...
    goBack = () => {
        sessionStorage.setItem("commentList", '');
        hashHistory.goBack();
    }
    //监听事件，点击系统返回键
    clickKeyback = () => {
    ...
    render() {
        ...
        return (
            <div className="designerComment">
                ...
                {
                    this.state.location_form == "WorksDetails" ? (
                        <NavBar
                            className="NewNavBar"
                            mode="light"
                            icon={<Icon type="left" size="lg" style={{ "color": "#a3a3a3" }} />}
                            onLeftClick={this.goBack}
                            rightContent={<Button className="rechargeButton" onClick={this.clickComment}>评论</Button>}
                        >评论列表</NavBar>
                    ) : null
                }
                {
